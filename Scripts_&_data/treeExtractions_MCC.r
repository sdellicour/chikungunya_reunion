treeExtractions_MCC = function(mcc_tre, mostRecentSamplingDatum)
	{
		mcc_tab = data.frame(matrix(nrow=dim(mcc_tre$edge)[1], ncol=5))
		colnames(mcc_tab) = c("node1","node2","length","startLoc","endLoc")
		mcc_tab[,c("node1","node2")] = as.numeric(mcc_tre$edge)
		mcc_tab[,c("length")] = as.numeric(mcc_tre$edge.length)
		for (i in 1:length(mcc_tre$annotations))
			{
				annotations = mcc_tre$annotations[[i]]
				if (!is.null(annotations$location.rate))
					{
						mcc_tab[i,"endLoc"] = annotations$location.rate
					}
			}
		for (i in 1:length(mcc_tre$annotations))
			{
				...
				if (length(index) > 0)
					{
						mcc_tab[i,"startLoc"] = mcc_tab[index,"endLoc"]
					}	else	{
						annotations = mcc_tre$root.annotation
						mcc_tab[i,"startLoc"] = annotations$location.rate
					}
			}
		...
		mcc_tab = rbind(mcc_tab1, mcc_tab2); mcc_tab = mcc_tab[,c(1,2,4,5,3,6:9)]
